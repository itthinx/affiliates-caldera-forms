...
// @todo  There is no explicit total field in Caldera Forms
// that can be used automatically for referral rate calculations

// @todo the first option is to use a select where the user will select one field among 
// the text and number fields present in the form.
		$form_fields       = null;
		$rate_field_select = '<select></select>';
		$caldera_form = Caldera_Forms_Forms::get_form( $id );write_log( $caldera_form['fields']);
...
// @todo The other option is to utilize a form field with the slug or label 'affiliates'
// and if this is not present, referral rate calculations won't be possible.
		$affiliates_amount_field = null;
		foreach ( $caldera_form['fields'] as $field_id ) {
			if ( $field_id['slug'] == 'affiliates' || $field_id['label'] == 'Affiliates' ) {
				$affiliates_amount_field = $field_id['label'];
			}
		}
